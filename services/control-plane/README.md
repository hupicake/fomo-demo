...
clean verification and publication pipeline. GoalGraph is the sole execution
path; historical P0 records remain readable but cannot start the retired SOP.
...
- Direct Pi model egress goes through LiteLLM with a per-run opaque virtual
  ...
  The legacy `fomo-pi-flash` alias remains only for persisted-run
  compatibility. Provider credentials stay in LiteLLM and never enter a
  sandbox.
- An uninterrupted run reuses one `fomo-pi-ds` session across planning,
  ...
  persisted usage balance. GoalGraph is always authoritative. BUILDING uses Pi's native
...
- The production `SandboxProvider` is `opensandbox` (OpenSandbox Server v0.2.2, SDK
...
The worker has no host-process fallback: an unavailable OpenSandbox boundary
fails closed. The FOMO system `.gitignore` baseline is owned by the control
plane and cannot be weakened by agent output.
...
receive LiteLLM/model or AWS credentials.
...
LiteLLM `4000`, random Preview host ports including `40000-60000`, and PostgreSQL
...
Tests use `FakeSandboxProvider` and fake runtime bridges, so they make no
network/model calls and execute no generated host code. Per
