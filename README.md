...
Goal panel are the production execution path. A reusable Pi session, automatic compaction, verified checkpoint
...
Before production rollout, validate every private Compose overlay against this
base file. Older overlays must remove `redis` and `minio` service extensions or
Compose will reject them now that those services no longer exist; keep any
compatibility Preview gateway while persisted legacy Preview URLs still use it.
...
OpenSandbox is the only production sandbox provider. If it is unavailable, the
run fails closed instead of falling back to a host process.
...
credential, or unrelated worker credential.
...
random generated-Preview ports (including `40000-60000`), or PostgreSQL `5432`.
Recreate older Compose containers after
